# -*- coding: utf-8 -*-
"""app.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1t923FYIs0-hIPLk2xT6SKKIUE98ltFeT
"""

# Cell 1: Install dependencies
!pip install streamlit pyngrok pandas matplotlib seaborn

!pip install streamlit pyngrok

from pyngrok import ngrok
ngrok.kill()  # This kills all active tunnels

# Cell 2: Mount Google Drive
from google.colab import drive
drive.mount('/content/drive')

import streamlit as st
import pandas as pd
import altair as alt

# Sample dataframe generation (replace this with your own dataset)
# Expected columns: 'Country', 'GHI', 'DNI', 'DHI'
@st.cache_data
def load_data():
    import numpy as np
    countries = ['Benin', 'Togo', 'Sierra Leone']
    data = []
    for country in countries:
        for _ in range(3000):
            data.append({
                'Country': country,
                'GHI': np.random.normal(220, 80),
                'DNI': np.random.normal(150, 100),
                'DHI': np.random.normal(110, 70)
            })
    return pd.DataFrame(data)

df = load_data()

# Sidebar - Filters
st.sidebar.header("🔍 Filter Options")
countries = st.sidebar.multiselect("Choose Countries", df["Country"].unique(), default=["Benin", "Togo", "Sierra Leone"])
metric = st.sidebar.selectbox("Select Metric for Boxplot", ["GHI", "DNI", "DHI"])
sample_size = st.sidebar.slider("Sample Size for Charts", 100, 3000, 1000)

# Main Title
st.markdown("## ☀️ Solar Insight Dashboard")
st.markdown("Explore solar radiation and climate metrics across selected West African countries.")

# Summary Statistics
st.markdown("### 📊 Summary Table")

summary = df[df["Country"].isin(countries)].groupby("Country").agg(
    {
        "GHI": ["mean", "std", "median"],
        "DNI": ["mean", "std", "median"],
        "DHI": ["mean", "std", "median"]
    }
).round(2)

summary.columns = ['_'.join(col).strip() for col in summary.columns.values]
summary.reset_index(inplace=True)

st.dataframe(summary, use_container_width=True)

# Boxplot Chart
st.markdown(f"### 📦 Distribution of {metric} by Country")

box_data = df[df["Country"].isin(countries)].sample(sample_size)

box_chart = alt.Chart(box_data).mark_boxplot(extent='min-max').encode(
    x=alt.X('Country:N', title="Country"),
    y=alt.Y(f'{metric}:Q', title=metric),
    color='Country:N'
).properties(
    width=700,
    height=400
)

st.altair_chart(box_chart, use_container_width=True)

# (Optional) Add bubble chart, temperature trends, humidity, etc. if desired

# deploy.py

# Install necessary packages
import os
import subprocess
import sys

# Install required packages only once
subprocess.run([sys.executable, "-m", "pip", "install", "-q", "streamlit", "pyngrok", "pandas", "matplotlib", "seaborn"])

from pyngrok import ngrok
from pathlib import Path

# Configuration
PORT = 8501
AUTH_TOKEN = "2xZlUxfhFUbZzDsSJ2KAvpeFMB2_2xFk4C4wUXP8mVkPtH3ci"  # Use your valid token here

# Validate data path
data_path = Path("/content/drive/MyDrive/Solar_Project")
if not data_path.exists():
    raise RuntimeError("❌ Google Drive not mounted. Please mount it before running.")

# Set ngrok auth token
ngrok.set_auth_token(AUTH_TOKEN)

# Start ngrok tunnel
public_url = ngrok.connect(addr=PORT)
print(f"🚀 Public URL: {public_url}")

# Start Streamlit app
print("📊 Starting Streamlit app...")
os.system(f"streamlit run app.py --server.port {PORT} --server.headless true")

